...
import com.ninjas.world.linkedlist.singly.polynomial.PolynomialNode;
...
public class LinkedList {
    public static PolynomialNode buildPolynomial(){
        Scanner scanner = new Scanner(System.in);
        int exponent = scanner.nextInt();
        double coeficient = scanner.nextDouble();
        PolynomialNode head = null,tail = null;
        while(exponent != -1){
            PolynomialNode node = new PolynomialNode(coeficient,exponent);
            if(null == head){
                head = node;
                tail = node;
            } else{
